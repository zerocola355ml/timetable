...
class ExamSchedulerApp:
    ...
    def __init__(self, config: Optional[ExamSchedulingConfig] = None, data_dir: str = "."):
        ...
        self.logger = get_logger('exam_scheduler_app')
    ...
    def load_all_data(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(f"Failed to load enrollment data, continuing without it: {e}")
                self.student_names = []
                self.enroll_bool = None
            ...
            self.logger.debug(f"Loaded exam_info date_periods keys: {list(self.exam_info.get('date_periods', {}).keys())}")
            for day, periods in self.exam_info.get('date_periods', {}).items():
                ...
                self.logger.debug(f"Day {day}: active={active_periods}, deleted={deleted_periods}")
            ...
            if self.enroll_bool is not None and len(self.student_names) > 0:
                ...
                self.logger.debug("No enrollment data available, creating empty student subjects")
                self.student_subjects = {}
    ...
    def _load_subject_constraints(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.debug(f"과목별 제약조건 로드 중 오류 발생: {e}")
            return {}
    ...
    def _load_teacher_slot_constraints(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.debug(f"교사 슬롯별 제약조건 로드 중 오류 발생: {e}")
            return {}
    ...
    def _load_subject_conflicts(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.debug(f"과목 충돌 제약조건 로드 중 오류 발생: {e}")
            return {}
    ...
    def _load_student_conflicts_with_priority(self) -> Dict[str, List[str]]:
        ...
        """
        self.logger.debug("Loading student conflicts with priority logic...")
        
        # 1순위: individual_conflicts.json
        individual_conflicts = self._load_json_file('individual_conflicts.json')
        if individual_conflicts:
            self.logger.debug(f"Using individual_conflicts.json ({len(individual_conflicts)} conflicts)")
        ...
        if same_grade_conflicts:
            self.logger.debug(f"Using same_grade_conflicts.json ({len(same_grade_conflicts)} conflicts)")
        ...
        try:
            ...
            if enrollment_conflicts:
                self.logger.debug(f"Using enrollment-based conflicts ({len(enrollment_conflicts)} subjects)")
                return enrollment_conflicts
            else:
                self.logger.debug("Enrollment conflicts is None or empty")
                return {}
        except Exception as e:
            self.logger.debug(f"Failed to load enrollment data: {e}")
            return {}
    ...
    def _convert_individual_to_conflict_dict(self, individual_conflicts: list) -> Dict[str, List[str]]:
        ...
        self.logger.debug(f"Converted individual_conflicts to conflict_dict with {len(conflict_dict)} subjects")
    ...
    def _convert_same_grade_to_conflict_dict(self, same_grade_conflicts: list) -> Dict[str, List[str]]:
        ...
        self.logger.debug(f"Converted same_grade_conflicts to conflict_dict with {len(conflict_dict)} subjects")
        return conflict_dict
    ...
    def create_schedule(self, time_limit: int = 120, status_callback=None) -> Tuple[str, Dict[str, Any]]:
        ...
        """
        try:
            self.logger.debug(f"Starting schedule creation with time_limit={time_limit}")
            self.logger.debug(f"exam_info keys: {list(self.exam_info.keys())}")
            
            # 1. 시험 슬롯 생성: /exam-info 편집 결과를 그대로 사용
            if status_callback:
                status_callback("시험 슬롯을 생성하고 있습니다...", 50)
                
            self.logger.debug("Creating exam slots...")
            try:
                slots = self.scheduler.create_slots(self.exam_info)
                self.logger.debug(f"Created {len(slots)} slots: {slots}")
                
                if not slots:
                    self.logger.debug("ERROR: No slots created!")
                    return "ERROR", {"error": "시험 슬롯이 생성되지 않았습니다. 시험 정보를 확인해주세요."}
            except ValueError as e:
                self.logger.debug(f"ERROR: Slot creation failed: {e}")
                return "ERROR", {"error": str(e)}
            
            self.logger.debug("Creating slot mappings...")
            try:
                slot_to_day, slot_to_period_limit = self.scheduler.create_slot_mappings(slots, self.exam_info)
            except ValueError as e:
                self.logger.debug(f"ERROR: Slot mapping failed: {e}")
                return "ERROR", {"error": str(e)}
            self.logger.debug(f"slot_to_day: {slot_to_day}")
            self.logger.debug(f"slot_to_period_limit: {slot_to_period_limit}")
            
            # 2. 모델 구축
            if status_callback:
                status_callback("최적화 모델을 구축하고 있습니다...", 60)
                
            self.logger.debug("Building optimization model...")
            # 어려운 과목 설정 로드
            hard_subjects = self._load_hard_subjects_config()
            self.logger.debug(f"Loaded hard_subjects: {hard_subjects}")
            ...
            self.logger.debug(f"스케줄 생성 중 오류 발생: {e}")
            import traceback
    ...
    def _load_hard_subjects_config(self) -> Dict[str, bool]:
        ...
        try:
            ...
            self.logger.debug(f"Warning: Failed to load hard subjects config: {e}")
            return {}
    ...
    def print_results(self, result: Dict[str, Any]):
        ...
        if 'slot_assignments' not in result:
            self.logger.debug("결과가 없습니다.")
            return
        
        self.logger.debug("=== 시험 시간표 배정 결과 ===")
        for slot, subjects in result['slot_assignments'].items():
            self.logger.debug(f"{slot}: {', '.join(subjects)}")
        
        # 요약 정보 출력
        summary = self.get_summary(result)
        self.logger.debug(f"\n=== 요약 정보 ===")
        self.logger.debug(f"총 학생 수: {summary['total_students']}명")
        self.logger.debug(f"총 과목 수: {summary['total_subjects']}개")
        self.logger.debug(f"총 슬롯 수: {summary['total_slots']}개")
        
        self.logger.debug(f"\n=== 하루 시험 수 분포 ===")
        for num, info in summary['exam_distribution'].items():
            self.logger.debug(f"{num}과목: {info['count']}명")
    ...
    def _load_fixed_assignments(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            if not os.path.exists(manual_schedule_file):
                self.logger.debug("No manual_schedule.json found, no fixed assignments")
                return {}
            
            with open(manual_schedule_file, 'r', encoding='utf-8') as f:
                content = f.read().strip()
                if not content:
                    self.logger.debug("Empty manual_schedule.json, no fixed assignments")
                    return {}
                
                data = json.loads(content)
                fixed_assignments = data.get('slot_assignments', {})
                
                self.logger.debug(f"Loaded fixed assignments from manual_schedule.json: {fixed_assignments}")
                return fixed_assignments
                
        except (json.JSONDecodeError, FileNotFoundError) as e:
            self.logger.debug(f"Failed to load manual_schedule.json: {e}")
            return {}
        except Exception as e:
            self.logger.debug(f"Unexpected error loading fixed assignments: {e}")
            return {}
    
    def set_use_fixed_assignments(self, use_fixed: bool):
        """고정 배치 사용 여부를 설정합니다."""
        self.use_fixed_assignments = use_fixed
        self.logger.debug(f"Fixed assignments usage set to: {use_fixed}")
...
def main():
    ...
    # 2. 데이터 로드
    app.logger.info("데이터를 로드하는 중...")
    if not app.load_all_data():
        app.logger.error("데이터 로드에 실패했습니다.")
        return
    
    app.logger.info("데이터 로드 완료!")
    
    # 3. 시험 시간표 생성
    app.logger.info("시험 시간표를 생성하는 중...")
    status, result = app.create_schedule(time_limit=120)
    
    if status == "SUCCESS":
        app.logger.info("시험 시간표 생성 완료!")
        app.print_results(result)
        app.save_results(result)
    else:
        app.logger.error(f"시험 시간표 생성 실패: {status}")
        if 'error' in result:
            app.logger.error(f"오류: {result['error']}")
